...
%------ Frontmatter ------
\title{DLMtool: Data-Limited Methods Toolkit (v2.0)}
...
\section{Efficacy test of an hypothetical Marine Reserve}
...
We use the Blue\_shark stock object as a template and modify two variables, the probability of individuals staying in area 1 (the marine reserve) and the size of area 1.
